...
unsigned int Ext2FS::get_block_address(struct Ext2FSInode * inode, unsigned int block_number)
{
	...
	*/ 
	...
	if (block_number < cant_LBA)
	{
		...
		unsigned char* buffer = (unsigned char*) malloc(block_size); // le pido espacio a dios y como solo le dije la cantidad de espacio le digo entre ()
		read_block(inode->block[block_number], buffer);
		uint tmp = buffer[block_number]; // recordemos que le sacamos 12 para poder usarlo de indice acá, usamos var esta para poder hacer el free
		free(buffer); // todo lo que se le pide a dios se le dueveulve
		return tmp;
	}
	...
	*/
	...
	if (block_number >= cant_LBA)
	{
		...
		if (block_number < cant_LBA*cant_LBA) // menor a canridad de bloques accesibles al cuadrado
		{
